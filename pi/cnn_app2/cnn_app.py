...
if __name__ == '__main__':
    ...
    for i in range(len(test_image)):
        ...
        print(result)
        print('\n')
        core_data = str(accr).replace('[', '').replace(']', '').split()
#         core_data = re.sub(r"\W", "", str(accr)).split()
        print(core_data)
        hit_data = [core_data.index(el) for el in core_data if float(el) >= 0.8]  # 0.8以上のスコアのインデックスを絞り込む
        print(hit_data)
        if len(hit_data) > 0:
            print('この画像に写っているフルーツは', end='')
            i = 0
            for el in hit_data:
                piriod = ', ' if i > 0 else ''
                if el == 0:
                    print(piriod + 'apple', end='')
                elif el == 1:
                    print(piriod + 'banana', end='')
                elif el == 2:
                    print(piriod + 'watermelon', end='')
            print('だと思われます')
            i += 1
